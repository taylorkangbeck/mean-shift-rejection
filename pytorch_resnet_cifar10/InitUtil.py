...
def LinearWeightInit ( ParamData, Method, UseZMI=False, UsePerFilterMagBalancing=False, InitMagnitudeScale=None ) :
    ...
    if UsePerFilterMagBalancing :  # DONT NEED
...
def ConvWeightInit ( ParamData, Method, UseZMI=False, UsePerFilterMagBalancing=False, InitMagnitudeScale=None, InitMaxScale=None ) :
    ...
    if UseZMI :
        ...
        if ParamShape[-1] == 1 :  # KEEP, might have to try this out with depthwise filter as dim(1,2) [MobileNet]
            ParamData -= torch.mean ( ParamData, dim=(1,), keepdim=True )
        else :
            ParamData -= torch.mean ( ParamData, dim=(2,3), keepdim=True )
        ###

        # mean subtracted mag  # RESTORING RANDOMNESS AND MAGNITUDE OF FILTER
    ...
    if InitMagnitudeScale is not None :  # remove if (InitMagnitudeScale should default to 1.)
        ParamData *= ( InitMagnitudeScale / ParamData.pow(2).sum(dim=PerFilterDims,keepdim=True).sqrt() )  # TODO PORT
...
### TODO PORT THIS VERSION:
def ConvWeightInit(ParamData, Method, UseZMI=False, UsePerFilterMagBalancing=False, InitMagnitudeScale=None,
                   InitMaxScale=None):
    """
    Convolutional layer weight parameter initialisation method taking parameters :
    ...
    # optionally each filter is rescaled to the specified Euclidean scale e.g. if originally Xavier or He with mag of 1 then rescaled to W' = W/mag(W)*InitMagnitudeScale
    #  (InitMagnitudeScale should default to 1.)
    ParamData *= (InitMagnitudeScale / ParamData.pow(2).sum(dim=PerFilterDims, keepdim=True).sqrt())  # TODO PORT
